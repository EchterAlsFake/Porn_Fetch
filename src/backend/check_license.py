...
class LicenseManager:
    ...
    def storage_path(self) -> Path:
        return self._storage_path # -> Usually in AppData / .local/
    ...
    def load_installed(self) -> LicenseCheckResult:
        ...
        try:
            ...
            data = json.loads(raw) # Loads the license file as json, so that we can check it
        except Exception:
    ...
    def validate(self, lic: dict[str, Any]) -> LicenseCheckResult:
        # Basic schema checks
        for k in ["schema", "product", "kid", "alg", "license_key", "session_id", "created_at", "features", "sig"]:
            if k not in lic:
